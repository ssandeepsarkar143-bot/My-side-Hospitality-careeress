...
- **Force re-login after Edit Promoted User (owner → admin)**: Owner's `saveEditPromotedUser` now (1) snapshots the previous role/authorities/locations, (2) computes a clean diff (`grantedAuths`, `revokedAuths`, `addedLocations`, `removedLocations`, `roleChanged`), (3) writes a `forceReauth` object onto the user doc only when something the admin can SEE actually changed, (4) creates a `roleUpdated` in-app notification with a human-readable summary in the bell tray. In admin-feed.html, `startForceReauthWatcher` listens via `onSnapshot(users/{uid})` and compares `forceReauth.at` to `auth.currentUser.metadata.lastSignInTime` (with a 1.5s clock-skew grace). If the edit happened after the last sign-in, a professional fullscreen overlay (amber theme, 🔐) shows: editor name, timestamp, role change, granted/revoked permission chips, added/removed location chips, plus a "Re-Login Now" button and a 10-second auto-logout countdown. Existing data is preserved — only what the admin can see changes (allocated-locations filter naturally hides the data of removed states on next login). Skipped for owner and for owner-preview-as-admin sessions.
