...
## AI Assistant (Gemini)
- `js/chatbot.js` shows a language picker (Auto + EN, HI, BN, TA, TE, MR, GU, PA, UR). Default is **Auto** — the AI auto-detects whatever language/script the user types in (English, Hindi, Bangla, Hinglish, Banglish, …) and replies in the same one. Picking a chip stores the choice in `localStorage` and is sent as `langPref` to the server, which appends a "USER LANGUAGE PREFERENCE" instruction to the system prompt.
- Both `server.js` (`/api/chat`, `/api/chat-stream`) and `cloudflare-worker/worker.js` accept `{ message, history, userRole, langPref }` and call `buildSystemPrompt(userRole, langPref)` which returns:
  - A mature, concierge-style **identity** with strict language rule and (for non-management) a **PRIVACY RULE** that hides revenue, real user counts, success %, admin lists, scheduler internals, MPINs and any other internal data.
  - Mature **fallback answers** for "feedback", "user count", "success rate", "launch", "story", "CEO", "co-founder", "states we operate in" — these are framed as attractive marketing-grade statements, never invented numbers or testimonials.
  - `PUBLIC_KB` (visible to everyone) + `MGMT_KB` (only owner/admin/sub-admin).
- Quick-action buttons are kept in English; greeting messages, language-confirm messages and the chip row support multiple Indian languages.
...
- **Cloudflare Workers (free plan)** — proxies all `/api/*` calls to Gemini and holds the API key as an encrypted secret. 100,000 requests/day free.

### Architecture
| Layer | Where | Cost |
|---|---|---|
| Frontend (HTML/CSS/JS) | Firebase Hosting | $0 |
| Auth + Firestore + Storage | Firebase Spark | $0 |
| AI proxy (`/api/chat`, `/api/chat-stream`, `/api/resume`, `/api/translate`) | Cloudflare Worker | $0 |
| In-app notifications (bell icon) | Firestore listener | $0 |
| Local development | Replit Express `server.js` (port 5000) | $0 |

The front-end auto-detects environment via `js/api-config.js`:
- On `localhost`, `*.replit.dev`, `*.replit.app`, `*.repl.co` → calls go to the local Express server (relative URLs).
- On Firebase Hosting domains → calls go to the Cloudflare Worker URL configured in `js/api-config.js` (`HC_AI_BASE_PROD`).

### One-time setup

**Step A — Deploy the Cloudflare Worker (5 minutes, no credit card):**
1. `npm install -g wrangler`
2. `wrangler login` (browser opens; sign up with email + password — no card asked)
3. `cd cloudflare-worker`
4. `wrangler secret put GEMINI_KEY` — paste your free Gemini key from https://aistudio.google.com/apikey
5. `wrangler deploy` — copy the resulting URL, e.g. `https://hospitality-gemini.<your-subdomain>.workers.dev`

See `cloudflare-worker/README.md` for full details, CORS lock-down, and verification curls.

**Step B — Wire the front-end to the Worker:**
6. Open `js/api-config.js` and replace the empty `HC_AI_BASE_PROD = ''` with your Worker URL from step 5.

**Step C — Deploy Firebase Hosting:**
7. `npm install -g firebase-tools`
8. `firebase login`
9. `firebase use hospitality-careers-e662f`
10. `firebase deploy --only hosting,firestore:rules,storage` — deploys static site + rules.

> **Note:** `firebase.json` no longer contains Cloud Functions rewrites (those required Blaze plan). The `functions/` folder is kept for reference only and is excluded from hosting via the `ignore` list.

### Verify AI Resume Builder works after deploy
1. Open `https://hospitality-careers-e662f.web.app/resume-builder.html`
2. Fill Full Name + Job Title + 1 experience + 1 skill → click **Generate My Resume**
3. Should show a formatted resume within 5–10s. If "Failed":
   - Browser DevTools Network tab → look for the `/api/resume` request — its URL should point to your Worker (`hospitality-gemini.*.workers.dev`), not your Firebase domain.
   - If 500 from the Worker: `wrangler tail` in `cloudflare-worker/` to see live logs. Most common cause is missing/wrong `GEMINI_KEY`.

### About OS lock-screen push notifications
True OS-level push (browser closed, lock-screen wake-up) needs a server process that calls FCM whenever a Firestore notification doc is written. That requires Cloud Functions (Blaze) **or** a paid always-on VM — neither is achievable on the $0 / no-card path.
**Current behavior:** the in-app bell icon delivers real-time notifications via Firestore listener whenever the page is open; PWA install gives a home-screen icon. Add Cloud Functions later when a card is available.
...
- **Preconnect hints** for every cross-origin resource the page actually needs: `fonts.googleapis.com`, `fonts.gstatic.com` (+ crossorigin), `www.gstatic.com` (Firebase SDK + Google sign-in icon), `firestore.googleapis.com` (DB calls), `cdnjs.cloudflare.com` (Font Awesome). Google's renderer can now open all 5 TLS sockets in parallel during HTML parse.
- **Inlined the Google sign-in icon** as an inline SVG on both Continue-with-Google buttons in `index.html`. Previously fetched from `https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg` — now zero network call, identical visual.
- **`crossorigin="anonymous" referrerpolicy="no-referrer"`** added to the Font Awesome stylesheet on both pages so Google's renderer can use the cached version cleanly without sending the page URL as a Referer header.
- **NOTE**: most of the "19/29 couldn't load" entries in Search Console URL Inspection are Firebase SDK chunks + Firestore REST queries that fail with permission-denied for unauthenticated Googlebot (firestore.rules require auth). This is expected and does NOT block indexing — Google still sees the static HTML, the H1, the textual content, and follows the internal links. The above changes simply reduce the count to make the warning less alarming.

## Recent Fixes (2026-04-29) — /jobs.html landing page + cross-site SEO wiring
...
- **`js/request-badges.js` (new, site-wide)**: live `onSnapshot` of unread `notifications` for the current user, broken down per `requestType`. Fills:
  - Owner sidebar: `#ownerBadge_jobPost / employerConnect / contact / resumeDownload / membership`
  - Admin sidebar: `#adminBadge_jobPost / employerConnect / contact / resumeDownload / membership`
  - Generic data attribute: `[data-req-badge="<type>"]`
  - Site-wide bell badges: `#notifBadge / #adminNotifBadge / #userNotifBadge`
  Wired into `owner-feed.html` and `admin-feed.html` after `maintenance-overlay.js`.
- **AI overhaul (server.js + chatbot.js)**:
  - `js/chatbot.js` re-enables a 10-language chip row (Auto, EN, HI, BN, TA, TE, MR, GU, PA, UR) and sends `langPref` with every chat call.
  - `buildSystemPrompt(userRole, langPref)` adds: mature concierge tone; full `LANGUAGE RULE` (auto-detect + explicit override + mid-conversation switch); strict `PRIVACY RULE` blocking revenue, real user counts, success %, admin lists, scheduler internals, MPINs, Firestore details; mature `ALWAYS-SAFE` answers for feedback, launch, our story, the Hospitality Careers management team, states we operate in, user count, success rate — never invented numbers.
  - `PUBLIC_KB` rewritten with HC Wallet, Refer & Earn, Coupons, Notifications, Group Chat and updated Prime/payment flow. `MGMT_KB` unchanged.
- **Full SEO**:
  - New `robots.txt` (allows all public marketing pages, blocks dashboards & APIs, declares the sitemap URL) and `sitemap.xml` (8 public URLs with priorities), both served from `server.js` with the right `Content-Type`. `firebase.json` adds matching headers for production (Firebase Hosting). `.local`-style ignore list left untouched so both files publish from the repo root.
  - `index.html` head: full meta description / keywords / canonical / Open Graph / Twitter card, **Google Search Console verification placeholder** (`<meta name="google-site-verification" content="YOUR_TOKEN_HERE"/>`), **Google Analytics 4 placeholder** (`G-XXXXXXXXXX`), and `application/ld+json` Organization + WebSite schema.
  - `index.html` body now includes a public, crawler-friendly section (`<h1>India's Trusted Hospitality Job Portal</h1>` + 4 feature cards + founder/year + nav) and a trust footer (security, PWA, copyright).
  - `about.html`, `membership.html`, `help.html`, `contact.html`, `feedback.html`, `resume-builder.html` all received per-page meta description, robots, canonical and Open Graph tags.
- **Search Console verification — manual step for the owner**: open Google Search Console → add property `https://findhospitalitycareers.web.app/` → choose "HTML tag" verification → copy the `content="..."` token → replace `YOUR_TOKEN_HERE` in `index.html` line ~30 → redeploy. Same idea for GA4: replace both occurrences of `G-XXXXXXXXXX` in `index.html` with the real GA4 Measurement ID.

## Recent Fixes (2026-04-28)
- **AI Job Match (user-feed + prime-feed)**: New "AI Job Match" card on both dashboards (mounted at `#aiMatchMount`, between HC Wallet and the Job Search filter) ranks live `jobPosts` against the candidate's profile and surfaces the top 5 picks with a 0–100 score badge, a one-line AI-generated reason, salary, location, department, accommodation icon, and an Apply button (predefined demo jobs offer a Scroll-to-Filter button instead). Powered by a new POST endpoint `/api/job-match` added to BOTH `cloudflare-worker/worker.js` (production) and `server.js` (local), accepting `{profile, jobs[], topK}` and returning `{matches:[{id,score,reason}], fallback?}`. Uses Gemini with `responseMimeType:application/json` for structured scoring; gracefully falls back to a deterministic keyword + city/state + salary scorer when Gemini quota is exhausted, key is missing, or the network fails. Client module `js/job-match.js` (ES module + `window.hcAttachJobMatch` global) auto-detects environment via `window.hcApiUrl()`, caches results in `localStorage` for 30 min keyed on profile fingerprint, includes a Refresh button to re-score, and gracefully degrades to a local in-browser keyword fallback if even the network is offline. If the user's profile lacks job title / skills / experience, the card prompts them to complete `profile.html` instead of showing fake matches. Worker README + endpoints table updated.
- **Location Comparison Dashboard (admin-feed)**: New sidebar item *"Location Comparison"* (under Management header, gated like Admin Working) opens a fresh section with a Chart.js 4.4 pie chart aggregating Jobs + Contacts + Resumes + Memberships + Hires across every in-scope sub-admin per state. A side-by-side per-state totals table mirrors the chart and shows admin count, total work, and share %. Clicking any pie slice (or table row) opens an inline drill-down panel listing every admin in that state with their per-counter breakdown and total. A green **Export CSV** button downloads the per-state aggregates. The pie reuses the same `__awScopedCache` produced by `loadAdminWorkingScoped` (so the same scoping rules and `_myPromotee` flag carry through), refreshes on demand, and shows a friendly empty state when no work has been recorded.
- **Op Mgr / Branch Head visibility fix (Admin Working scope)**: `loadAdminWorkingScoped` previously included only sub-admins whose allocated states overlapped the current admin's allocated states. Operations Managers and Branch Heads now ALSO see (a) every sub-admin they personally promoted (`promotedBy === currentUser.uid`), regardless of state overlap, and (b) when an OM/BH has no specific allocation, every sub-admin (their remit is org-wide). Personally-promoted admins are flagged with a 🌟 badge in the Admin Name column and in the Location Comparison drill-down so the manager can identify "their" people at a glance.
- **CSV Export — Admin Working Dashboard with weekly activity series**: Green "Export CSV" button next to the dashboard title produces `admin-working_<UTC-timestamp>.csv` with one row per sub-admin: Name, Email, Role, States (pipe-joined), Jobs, Contacts, Resumes, Memberships, Hired, 7-day Total, Trend (rising/steady/dropping/idle), plus seven `D_YYYY-MM-DD` columns containing the per-day activity counts pulled from the cached `series7d`. Reuses the in-memory cache so no extra Firestore reads are issued. Refresh button next to it re-runs the aggregation.
- **Group-chat bubble moved to navbar**: The floating chat icon is no longer just bottom-right (still available as a draggable fallback). Every feed (admin/owner/user/prime) now exposes a navbar-mounted `#navChatBtn` (gold theme, badge `#navChatBtnBadge`) right before the bell. `js/group-chat.js` v4 reveals the navbar button via the new `_mountNavbarButton()` helper as soon as the bubble would mount, mirrors the unread badge into it, and wires it to the same `togglePanel()` handler. Cache-busted all `import('./js/group-chat.js?v=4')` paths.

## Recent Fixes (2026-04-27)
- **Two-step Promotion-Request workflow (admin-feed → owner-feed)**: Promoted admins (Operations Manager, Branch Head, or any sub-admin holding the `promote` authority) can no longer apply promotions directly. The "Promote to Admin" modal now asks for a brief reason (`#ap_reason`) and on submit writes a pending document to a new `promotionRequests` collection (with duplicate-pending check, owner notification, audit log) instead of updating the user. A new sidebar item **"My Promotion Requests"** (badge `#myPromoReqBadge` for approved-not-yet-executed) lets the requester filter by Pending / Approved / Rejected, cancel a pending request, or click **Execute Promotion** on an approved one — which then performs the original promotion atomically (default MPIN 0000, notification, audit, marks `executed:true`). Owner-feed gets a matching **"Admin Promotion Requests"** section (`sec-promotionRequests`) defaulted to Pending, with one-click Approve/Reject; Reject requires a reason. Both badges are powered by live `onSnapshot` listeners. Notifications are sent to the requester on both decisions. Audit-log entries (`Promotion Request Submitted/Approved/Rejected`) preserve full attribution.
- **Per-row 7-day sparkline in admin Working Dashboard**: `loadAdminWorkingScoped` now iterates each work-source snapshot (instead of just `.size`) to build a daily activity series for the rolling last 7 days from `approvedAt` / `hiredAt` / `updatedAt` / `createdAt`. A new **"7d Trend"** column renders an inline 64×22 SVG bar chart per admin (rightmost = today, today bar in green, idle days in faint grey, declining trend in red), plus a 7-day total, "last X days ago" hint when idle today, and a ▼/● trend icon flagging admins whose recent half-week activity dropped below 60 % of the earlier half — perfect for at-a-glance spotting of activity drops without opening a per-user report.
- **Group-chat bubble drag is now fully free**: `js/group-chat.js` `_restoreFabPos` previously discarded any saved position above the upper 55 % of the viewport, snapping the bubble back down. It now simply *clamps* the saved position to stay on-screen, so the bubble behaves like the Replit AI agent icon — drag it anywhere on the page and it stays put. Drag wiring (`_setupDrag`) was already in place; no other changes needed.
- **Freeze-guard overlay copy**: replaced "contact the Owner" with a professional Management mention.
