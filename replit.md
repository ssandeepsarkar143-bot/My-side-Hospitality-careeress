...
- **AI overhaul (server.js + chatbot.js)**:
  ...
  - `buildSystemPrompt(userRole, langPref)` adds: mature concierge tone; full `LANGUAGE RULE` (auto-detect + explicit override + mid-conversation switch); strict `PRIVACY RULE` blocking revenue, real user counts, success %, admin lists, scheduler internals, MPINs, Firestore details; mature `ALWAYS-SAFE` answers for feedback, launch, our story, founder/CEO (Suman Sarkar), states we operate in, user count, success rate — never invented numbers.
